...
public final class UploadUtil {
    ...
    /** 上传文件临时目录 */
    private static final String uploadFileDir = "image/";
    ...
    /** 上传文件处理(支持批量) */
    public static Map<String,String> uploadImage(HttpServletRequest request, boolean lessen) {
        CommonsMultipartResolver multipartResolver = new CommonsMultipartResolver(
            request.getSession().getServletContext());
        ...
        if (multipartResolver.isMultipart(request)) {
            ...
            for (Iterator<String> iterator = multiRequest.getFileNames(); iterator.hasNext();) {
                ...
                if (multipartFile != null) {
                    String name = multipartFile.getOriginalFilename();
                    if (name.indexOf(".") == -1 && "blob".equals(name)) { //没有文件后缀则加上
                        name = name + ".png";
                    }
                    String uuid = UUID.randomUUID().toString();
                    String postFix = name.substring(name.lastIndexOf(".")).toLowerCase();
